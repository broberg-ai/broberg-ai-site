          ...
          <div class="f-liste" data-testid="featured-liste">
            {items.map((it) => (
              <a class="f-liste-kort" href={it.href} data-testid="featured-liste-kort">
                ...
                <b {...cmsAttrs({ collection: it.collection, slug: it.slug, locale }, it.collection === "posts" ? "title" : "name")}>{stripHtml(it.title)}</b>
                <p {...cmsAttrs({ collection: it.collection, slug: it.slug, locale }, it.collection === "posts" ? "featuredText" : it.collection === "platforms" ? "tagline" : "blurb")}>{it.featuredText}</p>
...
export async function renderPodcast(locale: Locale): Promise<string> {
  ...
  const data: PodcastData = {
    ...
    vaerter: [
      { initialer: "A", navn: g("podcastAidanNavn", "Aidan"), rolle: g("podcastAidanKort", isEn ? "explains" : "forklarer"), figur: "aidan" as const },
      { initialer: "Ai", navn: g("podcastAirinaNavn", "Airina"), rolle: g("podcastAirinaKort", isEn ? "asks on your behalf" : "spørger på dine vegne"), figur: "airina" as const },
    ],
